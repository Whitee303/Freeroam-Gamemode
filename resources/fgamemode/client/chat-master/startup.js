...
const chatWebView = new WebView("http://resource/client/chat-master/view/index.html");
...
function sendMessageToServer(message) {
    if (message.length > 0) emitServerRaw("vchat:message", message);
    unfocus();
}
...
function toggleChatWithKey(keyCode) {
    ...
    else if (keyCode === T_KEY && !chatWebView.focused && focusEnabled) focus();
}
