...
function send(player, message, type = MessageType.Default) {
    ...
    waitForMount(player).then(()=>emitClientRaw(player, "vchat:message", message, type)
...
function addSuggestion(player, suggestion) {
    ...
    waitForMount(player).then(()=>emitClientRaw(player, "vchat:addSuggestion", suggestion)
    ).catch(()=>log(`[vchat:addSuggestion] Timeout: Player ${player.name} is not mounted`)
    );
}
function toggleFocusEnabled(player, enabled) {
    waitForMount(player).then(()=>emitClientRaw(player, "vchat:focusEnabled", enabled)
    ).catch(()=>log(`[vchat:addSuggestion] Timeout: Player ${player.name} is not mounted`)
    );
}
...
function focus(player) {
    waitForMount(player).then(()=>emitClientRaw(player, "vchat:focus", true)
    ).catch(()=>log(`[vchat:addSuggestion] Timeout: Player ${player.name} is not mounted`)
    );
}
function focusAll() {
...
function unfocus(player) {
    waitForMount(player).then(()=>emitClientRaw(player, "vchat:focus", false)
    ).catch(()=>log(`[vchat:addSuggestion] Timeout: Player ${player.name} is not mounted`)
    );
}
function unfocusAll() {
...
function clear(player) {
    waitForMount(player).then(()=>emitClientRaw(player, "vchat:clear")
    ).catch(()=>log(`[vchat:addSuggestion] Timeout: Player ${player.name} is not mounted`)
    );
}
function clearAll() {
